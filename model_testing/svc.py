...
# %% Check data after outlier removal - Boxplot
...
# %% Check data after outlier removal - Distribution plots
g = sns.FacetGrid(melted_clean_df, col="Feature", col_wrap=3, height=4, sharey=False)
g.map_dataframe(sns.histplot, x="Value", hue="Quality", kde=True)
g.add_legend()
plt.tight_layout()
plt.show()
...
total_combos = len(C_values) * len(gamma_values) * len(kernel_types)
